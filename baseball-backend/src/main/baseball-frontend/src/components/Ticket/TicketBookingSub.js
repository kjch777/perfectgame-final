...
export const TicketBookingSub = () => {
  ...
  useEffect(() => {
    const loadCanClick = async () => {
      try {
        const response = await axios.get(`/api/ticket/checkSeatStatus`, {
          params: { gameCode },
        });
  ...
  const handleSeatClick = async (id, section) => {
    ...
    try {
        ...
        const checkSeatResponse = await axios.get(`/api/ticket/checkSeat`, {
            params: { seatId: id },
        });
        ...
        const lockSeatResponse = await axios.post(`/api/ticket/lockSeat`, {
            seatId: id,
            memberNo
        });

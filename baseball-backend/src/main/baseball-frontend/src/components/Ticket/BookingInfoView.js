...
export const BookingInfoView = () => {
    ...
    useEffect(() => {
        ...
        const memberStorage = localStorage.getItem("loginMember");
        
        if (!loginMember && memberStorage) {
            setLoginMember(JSON.parse(memberStorage));
        } else if (!loginMember) {
        ...
        if (loginMember) {
            const ticketData = async () => {
                try {
                    const response = await axios.get('/api/ticket/bookingInfoView', {
    ...
    const handleCancel = async () => {
        ...
        try {
            await axios.delete('/api/ticket/deleteTicket', {
        ...
        <div className="infoView-container">
            {ticketInfo.length > 0 ? (
                ...
                <div className="no-info-container">
                    <img src="/images/no-ticket.png" alt="No tickets" className="no-info-img" />
                    <h1>조회된 예매 내역이 없습니다.</h1>
                    <p>아직 예매를 진행하지 않으셨군요! 
                        <br/>
                    아래 버튼을 눌러 예매를 시작해 보세요.
                    </p>
                <Button onClick={() => navigate("/ticket/bookingMain")}>예매하기</Button>
            </div>
            )}
        </div>
    );
